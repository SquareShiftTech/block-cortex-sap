...
#
# SOURCE
# Table `bigquery-expertise.CORTEX_SAP_REPORTING.Languages_T002`
#
# REFERENCED BY
  ...
  derived_table: {
    sql: {% assign locale = _user_attributes['locale'] | replace: 'nb','no' | upcase | split: '_' %}
        ...
        SELECT LanguageKey_SPRAS AS Language_SPRAS,
               1 as rnk
        FROM `bigquery-expertise.CORTEX_SAP_REPORTING.Languages_T002`
        ...
        AND LanguageKey_SPRAS IN (
            ...
            FROM `bigquery-expertise.CORTEX_SAP_REPORTING.{{explore}}`)
        {% endif %}
        UNION ALL
